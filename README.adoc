...
The repository contains an Angular frontend, a Kotlin/Spring Boot backend, a Caddy static web server, an HAProxy/ModSecurity edge gateway, PostgreSQL, SeaweedFS/S3-compatible attachment storage and a local Keycloak OpenID Connect setup for Docker Compose based evaluation.
...
* HAProxy and ModSecurity/Nginx edge gateway that exposes the public HTTP/HTTPS ports and forwards `/api/*` requests to the backend service.
...
| Docker Compose starts the HAProxy/ModSecurity edge, frontend/Caddy, backend, PostgreSQL, SeaweedFS, Keycloak and the Keycloak database with persistent Docker volumes for application, object-storage and identity-provider state.
|===
...
| Edge Gateway
| Exposes the public HTTP/HTTPS ports, redirects HTTP to HTTPS, applies ModSecurity rules and forwards `/app`, `/api` and Keycloak paths to internal services.
| Inside Mail Support System
...
| Edge Gateway
| HAProxy and ModSecurity/Nginx
| Public HTTP/HTTPS entry point; forwards `/app/` to Caddy, `/api/*` to the backend and `/realms/*`, `/resources/*`, `/admin/*` to Keycloak.

| Backend API
| Kotlin 2.3, Java 21, Spring Boot 4.1, Spring Security OAuth2 Resource Server, Spring Data JPA, Spring Mail, AWS SDK S3
...
|
|-- ips/                     HAProxy public edge and rate-limiting entry point
...
| Backend
| Kotlin 2.3, Java 21, Spring Boot 4.1, Spring Security OAuth2 Resource Server, Spring Data JPA, Spring Validation, SpringDoc OpenAPI
...
| Build
| Gradle Kotlin DSL, Gradle wrapper 9.6.1, Node Gradle plugin, Node.js 24.12.0, npm 11.6.2

| Runtime
| Docker Compose, HAProxy, ModSecurity/Nginx, Caddy, Keycloak, Eclipse Temurin Java runtime
...
[source,env]
----
# Public edge ports exposed by the HAProxy/IPS gateway
HTTP_HOST_PORT=80
HTTPS_HOST_PORT=443
...
The Docker edge proxy expects a local TLS certificate. Generate it once when `proxy/cert/proxy.crt` or `proxy/cert/proxy.key` is missing:

[source,bash]
----
bash ./proxy/cert/generate-certificate.sh
----
