...
The review of the original application state and the resulting implementation direction is documented in link:docs/original-application-review.adoc[Original Application Review].
...
| Persistence
| PostgreSQL 16 for Docker Compose, H2 2.4.240 as fallback for isolated backend runs
...
Running the complete application locally requires Docker and Docker Compose. The local runtime depends on the Compose-managed PostgreSQL, SeaweedFS, Keycloak, frontend/Caddy, backend, WAF and IPS services.

Install the following tools:

* Docker and Docker Compose for the complete local application runtime
* Java 21 for Gradle build, check and orchestration tasks
* Node.js and npm only when working directly in `frontend/`
...
| Complete local application runtime
| Docker, Docker Compose and a root `.env` created from `.env.example`. Local infrastructure values can use the documented evaluation defaults. SMTP/IMAP mailbox settings and the support-mail address must be configured for the complete mail workflow.
...
| Backend module development only
| Java 21 and separately reachable infrastructure if the feature under development needs Keycloak, PostgreSQL, S3 storage or SMTP/IMAP.
| `./gradlew :backend:bootRun` starts only the Spring Boot process on the local development port. This is not a complete application startup path.

| Frontend module development only
| Node.js and npm when running the Angular development server directly outside Gradle.
| `cd frontend && npm ci && npm run start` starts only the Angular development server. Requests to `/api/*` are forwarded through `frontend/proxy.conf.json` and still require a reachable backend and identity provider.
...
Docker Compose reads runtime values from a root `.env` file. Create it from the template before running the system. Local PostgreSQL, SeaweedFS and Keycloak values in `.env.example` are non-secret evaluation defaults. Replace the SMTP/IMAP mailbox values and `SUPPORT_MAIL_ADDRESS` with the required support mailbox configuration before startup:
...
The `startLocal` task validates `.env` before Docker Compose starts and aborts with a clear message if required mailbox values still contain placeholders.
...
DB_USER=postgres
DB_PASSWORD=mail_system_local_db_password
DB_NAME=mail_system
...
STORAGE_S3_ACCESS_KEY=mail-system-local-access-key
STORAGE_S3_SECRET_KEY=mail-system-local-secret-key
STORAGE_S3_PATH_STYLE_ACCESS=true
STORAGE_S3_INITIALIZE_BUCKET=true

MAIL_SMTP_HOST=mailgate.thm.de
MAIL_SMTP_PORT=465
MAIL_SMTP_USERNAME=replace_with_thm_username
...
KEYCLOAK_DB_PASSWORD=mail_system_keycloak_db_password
----

For isolated backend development outside Docker, the backend has H2 defaults unless Spring datasource variables are exported explicitly. This mode is useful for backend implementation work and tests, but it does not provide the complete application runtime, browser login flow, edge gateway, Keycloak, PostgreSQL, SeaweedFS or mail infrastructure:
...
== Direct Module Development

[IMPORTANT]
====
Use `./gradlew startLocal` for the complete local application. The commands in this section start individual modules only and are intended for focused development/debugging when the required dependent services are already available separately.
====

=== Backend Only

Start only the backend process from the repository root with the Gradle wrapper. If mail credentials or external service settings are stored in `.env`, export them before starting the process:
...
The backend listens on http://localhost:8080 in this mode. Protected API behavior still depends on a reachable Keycloak issuer/JWK endpoint, and attachment or mail features need reachable storage and SMTP/IMAP services.

=== Frontend Only

Start only the Angular development server from the frontend module:
...
The frontend development server listens on http://localhost:4200. It uses `frontend/proxy.conf.json` so browser requests to `/api/*` are forwarded to the backend on http://localhost:8080. This is only a development convenience and does not replace the Docker edge gateway or Keycloak setup.
...
For local evaluation, `keycloak/realm-mail-support.json` imports matching Keycloak test identities for these sample profiles. Replace evaluation identities and credentials for real deployments.
...
=== Frontend cannot reach the API during module development

For the complete local application, access the frontend through https://localhost/app/ so the edge gateway can forward `/api/*` to the backend. When using the frontend development server separately, make sure the backend is reachable on http://localhost:8080 and restart `npm run start` so Angular loads `proxy.conf.json`.
